...
RED.workspaces = (function () {
    ...
    function addWorkspace (ws, skipHistoryEntry, targetIndex) {
        // 如果有flowId参数且是首次加载，跳过历史记录
        var isUrlFlowId = RED.urlHashParams && RED.urlHashParams.flowId;
        var shouldSkipHistory = skipHistoryEntry || isUrlFlowId;
        ...
        if (ws) {
            ...
            do {
                workspaceIndex += 1;
            } while ($("#red-ui-workspace-tabs li[flowname='" + RED._(RED.urlHashParams.protocolName, { number: workspaceIndex }) + "']").size() !== 0);
            ...
            ws = {
                ...
                locked: false,
                info: "",
                label: RED._(RED.urlHashParams.protocolName, { number: workspaceIndex }),
                env: [],
                hideable: true,
            };
            if (!skipHistoryEntry) {
    ...
    function createWorkspaceTabs () {
        ...
        if (path.startsWith('flow/')) {
            flowId = path.substring(5);
        }
        RED.urlHashParams = {
            flowId: flowId,
            protocolId: params.get('protocolId'),
            protocolName: params.get('protocolName')
        };
        console.log('hash获取参数', RED.urlHashParams);
        ...
        // 初始化流程加载逻辑
        if (RED.urlHashParams.flowId) {
            // 如果URL中有flowId参数，则加载该流程
            console.log('检测到flowId参数，将加载流程:', RED.urlHashParams.flowId);
            // 这里可以添加逻辑来加载指定ID的流程
        } else {
            // 如果没有flowId参数，则只新建一个空白流程，不加载其他流程
            console.log('未检测到flowId参数，将新建空白流程');
        }
    ...
    function init () {
        ...
        // 等待所有初始化完成后自动执行隐藏其他流程
        // setTimeout(function () {
        //     // 获取当前激活的工作空间（默认第一个）
        //     var currentActive = RED.workspaces.active();
        //     console.log('当前激活的工作空间:', currentActive)
        //     if (currentActive) {
        //         // 直接调用隐藏其他流程动作
        //         RED.actions.invoke("core:hide-other-flows", { id: currentActive });
        //     }
        // }, 2000);
        ...
        if (RED.urlHashParams.flowId) {
            // 如果有flowId，尝试加载指定流程
            console.log('根据URL参数flowId加载流程:', RED.urlHashParams.flowId);
            // 延迟执行以确保其他初始化完成
            setTimeout(function () {
                // 检查是否存在该流程
                var targetFlow = RED.nodes.workspace(RED.urlHashParams.flowId);
                if (targetFlow) {
                    // 切换到指定流程
                    RED.workspaces.show(RED.urlHashParams.flowId);
                    // 隐藏其他所有流程
                    RED.actions.invoke("core:hide-other-flows", { id: RED.urlHashParams.flowId });
                } else {
                    // 如果不存在，新建一个空白流程
                    console.log('指定的flowId不存在，新建空白流程');
                    var newFlow = addWorkspace();
                    RED.workspaces.show(newFlow.id);
                    // 隐藏其他所有流程
                    RED.actions.invoke("core:hide-other-flows", { id: newFlow.id });
                    // 立即保存
                    RED.actions.invoke("core:deploy-flows");
                    // 通知主程序
                    window.parent.postMessage({
                        type: RED.urlHashParams.flowId == 0 ? 'flow-create' : 'flow-modify',
                        data: JSON.stringify({
                            ...RED.urlHashParams,
                            flowId: newFlow.id
                        })
